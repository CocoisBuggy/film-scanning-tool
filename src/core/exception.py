class EosException(Exception):
    __error_codes: dict[str, int] = {
        ...
        "EDS_ERR_TAKE_PICTURE_RETRUCTED_LENS_NG": 0x00008D0F,
        "EDS_ERR_LAST_GENERIC_ERROR_PLUS_ONE": 0x000000F5,
    }

    __code_to_error = {v: k for k, v in __error_codes.items()}

    def __init__(self, code: int | None, func: str | None = None):
        if code is not None:
            # Call the base class constructor with the parameters it needs
            super().__init__(
                f"SDK {func if func is not None else ""} raised error {code} ({EosException.__code_to_error[code]})"
            )
...
def intercept(func):
    ...
    def wrapper(*args):
        err = func(*args)
        if err:
            raise EosException(err, func.__name__)
